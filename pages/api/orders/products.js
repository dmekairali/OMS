 ...
 * Fetches ONLY products from SKUWise-Orders sheet (optimized - no redundant order fetch)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ============================================
    // Load products from SKUWise-Orders sheet ONLY
    ...
    if (!productData || productData.length === 0) {
      console.log('⚠️ No data found in SKUWise-Orders sheet');
    ...
    if (orderProducts.length === 0) {
      console.log('⚠️ No products found for this order');
      return res.status(200).json({
        success: true,
        products: []
      });
    ...
    // ============================================
    // Map EACH product row using VERIFIED COLUMN INDICES
    // 
    ...
    products = orderProducts.map((row, idx) => {
      ...
      if (discountValue && discountValue !== '') {
        ...
        if (!isNaN(parsed) && isFinite(parsed)) {
          ...
          console.log(`⚠️ Invalid discount value for product ${idx + 1}: "${discountValue}", setting to 0`);
          discountPercent = 0;
        }
      }
